function Offspring = DEMMODE(Parents,Parameter)
...
%--------------------------------------------------------------------------
    ...
    if isa(Parents(1),'INDIVIDUAL')
        calObj  = true;
        Parent1 = Parents(1).decs;
        Parent2 = Parents(2).decs;
        Parent3 = Parents(3).decs;
        Parent4 = Parents(4).decs;
        Parent5 = Parents(5).decs;
    else
        calObj = false;
    end
    [N,D]  = size(Parent1);
    ...
    Site = rand(N,D) < CR;
    Offspring  = Parent1;
    Offspring2 = Parent1 + F*(Parent2-Parent3) + F*(Parent4-Parent5);
    
    xu = Global.upper;
    xl = Global.lower;
    
    % boundary restriction
    if ( any( xl > Offspring2 ) && any( Offspring2 > xu ) )
        Offspring2 = Parent1 - F*(Parent2-Parent3) - F*(Parent4-Parent5);
    end
    if ( any( xl > Offspring2 ) && any( Offspring2 > xu ) )
        Offspring2(Offspring2 < xl) = xl(Offspring2 < xl);
        Offspring2(Offspring2 > xu) = xu(Offspring2 > xu);
    end
    Offspring(Site) = Offspring2(Site);
    ...
    %% Select Offspring or Parent1
    Population = INDIVIDUAL(Parent1);
    Population(2) = INDIVIDUAL(Offspring);
    FrontNo = NDSort(Population.objs,Population.cons,2);
